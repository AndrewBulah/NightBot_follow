...
function generateFollowMessage(months, days) {
  const messages = [
    { days: 0, text: 'Здарова, залётный!' },
    { days: 1, text: 'Ты тут уже один день, ебать время летит!' },
    { days: 7, text: 'Уже целую неделю подписан на пикми-стримершу!' },
    { months: 1, text: 'Месяц херачишь в чате, фрик!' },
    { months: 2, text: 'Уже два месяца хихикаешь в чате!' },
    { months: 3, text: 'Несколько месяцев, нихуя себе. Понравилась стримерша?' },
    { months: 6, text: 'Полгода вместе! Красавчик!' },
    { years: 1, text: 'Целый год в подвале ХЫХАнтера!' },
...
app.get('/followage', async (req, res) => {
  try {
    ...
    if (!userId || !channelId) {
      return res.send(`🌀 Пользователь ${userLogin} ещё не попал на цепь в подвале стримера...`);
    }
    ...
    res.send('⚠️ ошибка, попробуй позже! (а лучше отъебись от меня. P.s. твой бот <3 ');
  }
});
